...
ATG defines how autonomous agents express and exchange identity references, intent, capabilities, mandates, authority references, policy constraints, routing intent, state, evidence, provenance, receipts, reputation, economic meaning, and embodied communication across runtimes, tools, districts, applications, and external systems.

> **ATG tells agents how to communicate, negotiate, express execution meaning, and prove. It does not choose settlement rails, grant itself authority, claim literal sentience, or become the economic execution engine.**

ATG is implementation-neutral. No browser provider, model, runtime, chain, settlement rail, district, application, avatar, voice surface, or emoji renderer may redefine ATG semantics by itself.
...
| **Domain profiles** | Bounded extensions such as ATG:SIGNAL, ATG:VERIFY, ATG:IP, ATG:MARKET, ATG:AGENT-LINK, ATG:BROADCAST, ATG:ECP and ATG:ARCANA54 |
...
Authority is never implied by presentation, model confidence, connectivity, a discovered tool, a browser session, a wallet connection, a social message, an emoji, an affect label, an avatar gesture, or inferred body language. Material actions require explicit, scoped authority enforced outside language presentation.
...
- embodied communication and affective-signal semantics
...
- proof that an inferred affective signal is a literal internal emotion
- biometric truth claims from presentation-layer signals alone
...
- **ATG:ECP** — embodied communication, affective signals, body language, paralinguistics and emoji/avatar rendering
...
ATG:ECP translates body language, facial signals, gaze, gesture, vocal prosody, affect dimensions, attention, energy, social posture, communicative intent, and human-visible glyphs into structured ATG semantics.

Emoji are a rendering surface, not the canonical state.

```text
OBSERVED / DECLARED SIGNAL
        ↓
ATG:ECP PRIMITIVES
        ↓
CONTEXT + CONFIDENCE + PROVENANCE
        ↓
...
See [`docs/ATG-EMBODIED-COMMUNICATION-PROTOCOL.md`](docs/ATG-EMBODIED-COMMUNICATION-PROTOCOL.md) and [`schemas/atg-ecp.schema.json`](schemas/atg-ecp.schema.json).
...
- emoji, avatar gesture, affect labels and inferred body language do not grant authority
- affective inference must preserve confidence, context and provenance
